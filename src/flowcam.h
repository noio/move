...
public:
    ...
    cv::Mat getFlowHigh() const
    {
        return flow_high;
    };
    ofVec2f getFlowAt(float x, float y)
    {
        return ofxCv::toOf(flow.at<cv::Vec2f>(y, x));
    };
    cv::Mat getFrame() const
    {
        return frame;
    };
    ofImage& getScreenTexture()
    {
        return frame_screen_im;
    };
    ofImage& getFlowHighTexture()
    {
        return flow_high_im;
    };
    const vector<ofPolyline>&  getContoursHigh() const
    {
        return contourfinder_high.getPolylines();
    };
